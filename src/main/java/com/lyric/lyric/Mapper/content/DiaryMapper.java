...
public interface DiaryMapper {
    ...
    @Insert("INSERT INTO diary(title, content, content_type, content_format, is_deleted, is_draft, emotion_score, word_count, writing_start_time, writing_end_time, writing_duration, diary_date) " +
            "VALUES(#{title}, #{content}, #{contentType}, #{contentFormat}, #{isDeleted}, #{isDraft}, #{emotionScore}, #{wordCount}, #{writingStartTime}, #{writingEndTime}, #{writingDuration}, #{diaryDate})")
     ...
     * @return 影响的行数
     */
    @Update("UPDATE diary SET title=#{title}, content=#{content}, content_type=#{contentType}, content_format=#{contentFormat}, " +
