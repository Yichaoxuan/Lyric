...
public class GetWeatherService {
    ...
    private WeatherInformation parseWeatherResponse(JsonNode root, String locationId, LocalDateTime date) {
        ...
        for (JsonNode hour : hourlyNode) {
            ...
            // 00:00"
            log.debug("比较时间：API 时间={}, 日记时间={}", time, date);
            ...
            if (DateTimeUtils.isSameTime(time, date)) {
                ...
                log.info("找到匹配的时间点：{}, 天气状况：{}, 天气图标：{}", time, weatherCondition, weatherIcon);
                break;
            }
        }
        ...
        if (weatherCondition == null || weatherIcon == null) {
            ...
            // 尝试从每日数据中获取天气状况和图标
            if (resultNode.has("iconDay")) {
                weatherIcon = resultNode.path("iconDay").asText();
            }
            if (resultNode.has("textDay")) {
                weatherCondition = resultNode.path("textDay").asText();
            }
        }
