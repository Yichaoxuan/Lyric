...
public class PersonsParsingService {
     ...
     * <ol>
     *     <li>遍历所有新人物信息</li>
     *     <li>一级匹配：按性别筛选候选人物</li>
     *     <li>二级匹配：在性别匹配的基础上，按名称和别名精确匹配</li>
     *     <li>三级匹配：在名称匹配的基础上，按关系匹配</li>
     *     <li>四级匹配：使用 AI 分析进行最终匹配（当存在多个候选时）</li>
     ...
     * <p>注意:</p>
     * <ul>
     *     <li>如果名称或候选列表为 null，直接返回空列表</li>
     *     <li>使用 Stream API 进行过滤，提高代码可读性</li>
     * </ul>
     *
     * @param name 待匹配的人物名称
     ...
     * <p>处理步骤:</p>
     * <ol>
     *     <li>调用 personUpdater 更新人物信息（别名、关系、性格、出现时间、出现次数）</li>
     *     <li>调用 createDiaryPersonRelation 创建日记与人物的关联关系</li>
     *     <li>从新人物信息中提取索引（如果有），存入 resultMap</li>
     * </ol>
     *
     * <p>说明:</p>
     * <ul>
     *     <li>此方法用于已存在人物的情况，会更新现有记录而非创建新记录</li>
     *     <li>索引用于保持 AI 分析时的人物顺序，便于前端展示或其他后续处理</li>
     *     <li>如果新人物信息中没有索引，默认使用 0</li>
     * </ul>
     *
     ...
     *
     * <p>示例输出:</p>
     * <pre>张三 - 性别匹配：候选人数量=3, 列表=[张三，李四，王五]</pre>
     * <pre>李四 - 名称匹配：候选人数量=1, 列表=[李四]</pre>
     * <pre>王五 - 关系匹配：候选人数量=0, 列表=[无]</pre>
     *
     * @param name 人物名称
